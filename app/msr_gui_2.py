...
class MyApp:
	...
	def run_buttonClick(self):
		...
		ffs_result = system("msr_2.py %s %s %s %s %s" % (self.msr_begin, self.msr_end, self.ffs_profile_file, self.ffs_services_file, '2'))
		crt_result = system("msr_2.py %s %s %s %s %s" % (self.msr_begin, self.msr_end, self.crt_profile_file, self.crt_services_file, '1'))
